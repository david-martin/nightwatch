...
module.exports = function(Nightwatch) {
  ...
  Actions.element = function(using, value, callback) {
    ...
    if(using === 'jquery') {
      return new elementsByJQuery(Nightwatch, true).command(value,
        function(result) {
          callback(result);
        }
      );
  ...
  Actions.elementIdDisplayed = function(using, id, selector, callback) {
    ...
    if(using === 'jquery'){
      ...
      return this.executeAsync(
        ...
        [],
        function() {
          return self.execute(function(selector) {
            var el = Nightwatch_jQuery(selector).get(0);
            if(el){
              return Nightwatch_jQuery(el).is(":visible");
            } else {
              return false;
            }
          },
